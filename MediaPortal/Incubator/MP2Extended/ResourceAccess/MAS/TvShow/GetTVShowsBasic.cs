...
namespace MediaPortal.Plugins.MP2Extended.ResourceAccess.MAS.TvShow
{
  ...
  internal class GetTVShowsBasic : IRequestMicroModuleHandler
  {
    public dynamic Process(IHttpRequest request, IHttpSession session)
    {
      ...
      {
        var episodeAspect = MediaItemAspect.GetAspect(item.Aspects, EpisodeAspect.Metadata);
        int index = output.FindIndex(x => x.Title == (string)episodeAspect[EpisodeAspect.ATTR_SERIES_NAME]);
        if (index == -1)
        {
          var episodesInThisShow = items.ToList().FindAll(x => (string)MP2ExtendedUtils.GetAttributeValue(x.Aspects, EpisodeAspect.ATTR_SERIES_NAME) == (string)episodeAspect[EpisodeAspect.ATTR_SERIES_NAME]);
          ...
          necessaryMIATypes.Add(MediaAspect.ASPECT_ID);
          MediaItem show = GetMediaItems.GetMediaItemByName((string)episodeAspect[EpisodeAspect.ATTR_SERIES_NAME], necessaryMIATypes);

          if (show == null)
          {
            Logger.Warn("GetTVShowsBasic: Couldn't find show: {0}", (string)episodeAspect[EpisodeAspect.ATTR_SERIES_NAME]);
            continue;
          }
