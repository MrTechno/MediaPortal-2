  ...
  public class TranscodeVideoMetadataExtractor : IMetadataExtractor
  {
    ...
    private void ConvertMetadataToAspectData(MetadataContainer info, IDictionary<Guid, IList<MediaItemAspect>> extractedAspectData)
    {
      ...
      foreach (AudioStream audio in info.Audio)
      {
        ...
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_STREAM, audio.StreamIndex.ToString());
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_CODEC, audio.Codec.ToString());
        if (audio.Language == null)
        {
          aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_LANGUAGE, "");
        }
        else
        {
          aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_LANGUAGE, audio.Language);
        }
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_BITRATE, audio.Bitrate.ToString());
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_CHANNEL, audio.Channels.ToString());
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_FREQUENCY, audio.Frequency.ToString());
        aspect.SetAttribute(TranscodeItemVideoAudioAspect.ATTR_DEFAULT, audio.Default ? "1" : "0");
      ...
      foreach (SubtitleStream sub in info.Subtitles)
      {
        ...
        if (sub.IsEmbedded)
        {
          aspect.SetAttribute(TranscodeItemVideoEmbeddedAspect.ATTR_SUBSTREAM, sub.StreamIndex.ToString());
          aspect.SetAttribute(TranscodeItemVideoEmbeddedAspect.ATTR_SUBCODEC, sub.Codec.ToString());

          if (sub.Language == null)
          {
            aspect.SetAttribute(TranscodeItemVideoEmbeddedAspect.ATTR_SUBLANGUAGE, "");
          }
          else
          {
            aspect.SetAttribute(TranscodeItemVideoEmbeddedAspect.ATTR_SUBLANGUAGE, sub.Language);
          }

          aspect.SetAttribute(TranscodeItemVideoEmbeddedAspect.ATTR_SUBDEFAULT, sub.Default ? "1" : "0");
          MediaItemAspect.AddOrUpdateAspect(extractedAspectData, aspect);
        }
      }
    }
